...
const toBytes = (hash: string): Uint8Array => {
...
export const naturalSize = (hash: string): { width: number; height: number } => {
...
/**
 * The average colour of the image as 0-255 RGB, read straight from the hash's DC terms (the
 * reference thumbHashToAverageRGBA). This drives the artwork-tinted page backgrounds.
 */
export const averageColor = (hash: string): [number, number, number] => {
  const { lDC: l, pDC: p, qDC: q } = header(toBytes(hash))
  const b = l - (2 / 3) * p
  const r = (3 * l - b + q) / 2
  const g = r - q
  const to255 = (v: number) => Math.round(255 * Math.max(0, Math.min(1, v)))
  return [to255(r), to255(g), to255(b)]
}

const cosTable = (n: number, size: number): Float64Array => {
...
export const decode = (hash: string, width: number, height: number): Uint8ClampedArray => {
