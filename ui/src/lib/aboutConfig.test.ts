...
describe('formatTomlValue', () => {
  ...
  it('formats different types of arrays correctly', () => {
    // String array
    expect(formatTomlValue('["genre", "tcon", "©gen"]')).toBe('[ "genre", "tcon", "©gen" ]')
  ...
  it('handles strings with backslashes and quotes', () => {
    expect(formatTomlValue('C:\\Program Files\\app')).toBe('"C:\\\\Program Files\\\\app"')
    expect(formatTomlValue('path\\to"file')).toBe('"path\\\\to\\"file"')
    expect(formatTomlValue('backslash\\ and "quote"')).toBe('"backslash\\\\ and \\"quote\\""')
...
describe('separateAndSortConfigs', () => {
  ...
  it('skips ConfigFile entries', () => {
    ...
    expect(result.regularConfigs).toEqual([{ key: 'RegularKey', value: 'value' }])
  ...
  it('skips ConfigFile entries with nested object input', () => {
    ...
    expect(result.devConfigs).toEqual([{ key: 'DevFlag', envVar: 'ND_DEVFLAG', value: 'true' }])
  })
